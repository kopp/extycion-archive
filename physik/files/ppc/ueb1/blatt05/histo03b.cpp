 ...
 * histo03b.cpp
 *
 * erstellt histogramm aus DOUBLE-zufallszahlen von stdin
 *
 * Parameter 
 *   <anzahl> [<anz. bins>]
 *
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	//min max finden
	double max=0; //Groesste Zufallszahl
	double min=0; //Kleinste Zufallszahl
	...
	for( int i = 0; i < anzahl; i++ )
	{
		...
		int bin = ( (x[i]-min) * unterteilungen )/(max-min) ;
		histo[ bin ]++;
		if( verbose ) std::cout << "# " << i << " von " << anzahl << " : " << x[i] << " kommt in Bin " << bin <<"\n";
	}
